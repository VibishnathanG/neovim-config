...
show_help = function()
	...
	local help_lines = {
		...
		"  ║                                                                         ║",
		"  ║             ⚡ KEYBINDINGS QUICK REFERENCE (F1) ⚡                      ║",
		"  ╚═════════════════════════════════════════════════════════════════════════╝",
		" ",
		"  📂 FILE NAVIGATION:",
		...
		" ",
		"  🗑️ DELETE (no clipboard):",
		"    d, D, x, c           → Delete without clipboard",
		"    cc                   → Delete the entire line and start insert with indent",
		" ",
		"    F1                   → Show this help",
		" ",
		...
		" ",
		"  🔍 INTERACTIVE RENAME (F2):",
		"    F2   or use ALT + R to replace all  → Rename current word interactively",
		"    Usage:",
		"      1. Place cursor on a word",
		"      2. Press F2",
		"      3. Enter replacement text",
		"      4. Confirm occurrences:",
		"           y = replace",
		"           n = skip",
	...
	-- Apply custom syntax highlighting for help_reference
	vim.bo.filetype = "help_reference"
	vim.cmd("syntax clear")
	vim.cmd("syntax match HelpHeader '^[╔║╚═].*$'")
	vim.cmd("syntax match HelpCategory '^\\s*[^A-Za-z0-9_ -]\\+\\s\\+[A-Z &]\\+:'")
	vim.cmd("syntax match HelpKey '^\\s\\+[^→]\\{-}\\s*→'")
	vim.cmd("syntax match HelpArrow '→'")
	vim.cmd("syntax match HelpBorder '──\\+'")
	vim.cmd("syntax match HelpBullet '●'")
	vim.cmd("highlight link HelpHeader String")
	vim.cmd("highlight link HelpCategory Title")
	vim.cmd("highlight link HelpKey Keyword")
	vim.cmd("highlight link HelpArrow Operator")
	vim.cmd("highlight link HelpBorder Comment")
	vim.cmd("highlight link HelpBullet Special")
	...
	vim.notify(
		"📘 Help opened — Press 'q' or Esc to close. If started from welcome, the startup menu will return.",
		vim.log.levels.INFO
	)
end
...
if vim.g.vscode then
	vim.keymap.set({ "n", "i" }, "<C-a>", function()
		require("vscode").action("editor.action.selectAll")
	end)
else
...
map("n", "<C-c>", function()
	vim.cmd.normal({ 'ggVG"+y', bang = true })
...
-- Normal mode → cut entire file
map("n", "<C-x>", function()
	vim.cmd.normal({ 'ggVG"+d', bang = true })
	vim.notify("✂️ Entire file cut to system clipboard.", vim.log.levels.INFO)
end)
...
map("n", "<C-z>", function()
	...
	vim.notify("🗑️ Entire file deleted (blackhole register).", vim.log.levels.INFO)
end)

-- Visual mode
map("v", "<C-z>", '"_d', { silent = true })
---------------------------------------------------
-- SAVE CURRENT FILE
---------------------------------------------------
map("n", "<C-s>", function()
...
local function replace_everywhere()
	...
	if string.find(text, "\n") then
		display_text = string.gsub(text, "\n", "\\n")
	end
	if #display_text > 30 then
		display_text = string.sub(display_text, 1, 27) .. "..."
	end
	local replacement = vim.fn.input("Replace '" .. display_text .. "' with: ")
	if replacement == "" then
		return
	end
...
vim.keymap.set({ "n", "i", "v" }, "<F2>", function()
	...
	-- Ctrl+Q aborts replace prompt
	vim.cmd([[
		cnoremap <C-q> q<CR>
	]])

	vim.cmd(string.format("%%s/\\<%s\\>/%s/gc", vim.pesc(old), vim.fn.escape(new, "/\\&")))
